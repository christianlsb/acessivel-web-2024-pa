...
const CadpcdForm = () => {
  ...
  const onSubmit = async (event: any) => {
    ...
    try {
      const response = await fetch("http://localhost:2424/queixante/patch/cadpcd", {
        method: "PATCH",
        headers: {
  ...
  return (
      <form onSubmit={onSubmit} className="p-10 bg-formContainer">
        ...
        <div className={"flex mt-12 gap-4"}>
          <Button type="button" className={"w-full bg-primaryBlue rounded-md text-center"}>
            <Link href={"/dashboard"}>Cancelar</Link>
          </Button>
          <Button  className={"w-full bg-primaryBlue"} disabled={cadPcdBool} type="submit">Cadastrar</Button>
        </div>
      </form>
  );
};
